...
namespace Manifest::Core::Graphics {
...
class Camera {
    ...
    void update_view_matrix() const {
        if (view_dirty_) {
            view_matrix_ = Math::look_at(position_, target_, up_);
            view_dirty_ = false;
    ...
    void update_projection_matrix() const {
        if (projection_dirty_) {
            projection_matrix_ = Math::perspective(fov_, aspect_ratio_, near_plane_, far_plane_);
