...
class Vector {
   ...
   public:
    ...
    constexpr Vector(T value) { data_.fill(value); }
    ...
    typename std::enable_if<std::is_floating_point<U>::value, T>::type length() const noexcept {
        return std::sqrt(length_squared());
    }

    template <typename U = T>
    typename std::enable_if<std::is_floating_point<U>::value, Vector>::type normalized()
        const noexcept {
        T len = length();
    ...
    // Swizzling helpers
    template <std::size_t M = N>
    typename std::enable_if<M >= 2, Vector<T, 2>>::type xy() const noexcept {
        return {data_[0], data_[1]};
    }

    template <std::size_t M = N>
    typename std::enable_if<M >= 3, Vector<T, 3>>::type xyz() const noexcept {
...
class Vector<float, 4> {
   ...
   public:
    ...
    float length_squared() const noexcept { return dot(*this); }
