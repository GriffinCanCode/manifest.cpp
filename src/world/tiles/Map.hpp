...
struct HexCoordinateHash {
    std::size_t operator()(const HexCoordinate& coord) const noexcept {
        return std::hash<std::int32_t>{}(coord.q) ^
               (std::hash<std::int32_t>{}(coord.r) << 1) ^
...
class TileMap {
    ...
    std::int64_t get_bucket_key(const HexCoordinate& coord) const noexcept {
        ...
        return (static_cast<std::int64_t>(bucket_q) << 32) | 
               static_cast<std::int64_t>(bucket_r);
    }
    ...
    void remove_from_spatial_index(TileId id, const HexCoordinate& coord) {
        ...
        bucket.tile_ids.erase(
            std::remove(bucket.tile_ids.begin(), bucket.tile_ids.end(), id),
            bucket.tile_ids.end()
        );
    }
...
public:
    ...
    static HexCoordinate neighbor_coordinate(const HexCoordinate& coord, Direction direction) noexcept {
    ...
    // Query methods
    std::vector<TileId> get_tiles_in_radius(const HexCoordinate& center, std::int32_t radius) const {
    ...
    std::vector<TileId> get_tiles_in_range(const HexCoordinate& min_coord, const HexCoordinate& max_coord) const {
        std::vector<TileId> result;
    ...
    auto tiles() const noexcept {
        return tiles_ | std::views::values | std::views::transform([](const auto& ptr) -> const Tile& {
            return *ptr;
        });
    }
    
    auto tiles() noexcept {
        return tiles_ | std::views::values | std::views::transform([](const auto& ptr) -> Tile& {
            return *ptr;
        });
    }
    ...
    std::pair<HexCoordinate, HexCoordinate> get_bounds() const noexcept {
        ...
        return {
            HexCoordinate{min_q.q, min_r.r, min_s.s},
            HexCoordinate{max_q.q, max_r.r, max_s.s}
        };
    }
