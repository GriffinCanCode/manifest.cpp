...
public:
    ...
    // Neighbor management
    void add_neighbor(ProvinceId neighbor) {
        neighbors_.insert(neighbor);
    }
    
    void remove_neighbor(ProvinceId neighbor) {
        neighbors_.erase(neighbor);
    }
    
    const std::unordered_set<ProvinceId>& neighbors() const noexcept { 
        return neighbors_; 
    }
    ...
    // Economic totals
    Core::Types::Production total_food() const { update_if_needed(); return total_food_; }
    Core::Types::Production total_production() const { update_if_needed(); return total_production_; }
    Core::Types::Production total_science() const { update_if_needed(); return total_science_; }
    Core::Types::Production total_culture() const { update_if_needed(); return total_culture_; }
    Core::Types::Production total_faith() const { update_if_needed(); return total_faith_; }
    Core::Types::Money total_trade() const { update_if_needed(); return total_trade_; }
    ...
    bool is_valid() const noexcept {
        return id_.is_valid() && !tiles_.empty();
    }
...
private:
    std::vector<TileId> select_province_seeds(const TileMap& tile_map, std::size_t count) {
        ...
        for (const auto& candidate : candidates) {
            ...
            for (TileId existing_seed : seeds) {
                ...
                std::int32_t distance = TileMap::distance(candidate_tile->coordinate(),
                                                          existing_tile->coordinate());
    ...
    float calculate_province_seed_score(const Tile& tile) {
        ...
        switch (tile.terrain()) {
            case TerrainType::Grassland: score += 2.0f; break;
            case TerrainType::Plains: score += 1.5f; break;
            case TerrainType::Hills: score += 1.0f; break;
            case TerrainType::Forest: score += 1.0f; break;
            case TerrainType::Coast: score += 0.5f; break;
            default: break;
        }
