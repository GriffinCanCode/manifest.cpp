...
class Region {
   ...
   public:
    ...
    // Neighbor management
    void add_neighbor(RegionId neighbor) { neighbors_.insert(neighbor); }

    void remove_neighbor(RegionId neighbor) { neighbors_.erase(neighbor); }

    const std::unordered_set<RegionId>& neighbors() const noexcept { return neighbors_; }
    ...
    float average_rainfall() const {
        update_if_needed();
        return average_rainfall_;
    }
    float average_fertility() const {
        update_if_needed();
        return average_fertility_;
    }

    // Economic totals
    Production total_food() const {
        update_if_needed();
        return total_food_;
    }
    Production total_production() const {
        update_if_needed();
        return total_production_;
    }
    Production total_science() const {
        update_if_needed();
        return total_science_;
    }
    Production total_culture() const {
        update_if_needed();
        return total_culture_;
    }
    Production total_faith() const {
        update_if_needed();
        return total_faith_;
    }
    Money total_trade() const {
        update_if_needed();
        return total_trade_;
    }
    ...
    bool is_valid() const noexcept { return id_.is_valid() && !provinces_.empty(); }
    ...
    std::vector<ProvinceId> select_region_seeds(const ProvinceMap& province_map,
        ...
        for (const auto& candidate : candidates) {
            ...
            for (ProvinceId existing_seed : seeds) {
                ...
                float distance =
                    distance(candidate_province->center(), existing_province->center());
...
class HierarchicalMap {
   ...
   public:
    ...
    RegionId get_region_for_tile(TileId tile_id) const {
        ...
        return province.is_valid() ? region_map_.get_region_for_province(province)
                                   : RegionId::invalid();
    }
