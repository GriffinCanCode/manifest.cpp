...
public:
    explicit Tile(TileId id, const HexCoordinate& coord) 
        : data_{.id = id, .coordinate = coord} {
    ...
    bool is_water() const noexcept {
        return data_.terrain == TerrainType::Ocean || 
               data_.terrain == TerrainType::Coast ||
               data_.terrain == TerrainType::Lake;
    }
    
    bool is_land() const noexcept {
        return !is_water();
    }
    ...
    float movement_cost() const noexcept {
        ...
        switch (data_.terrain) {
            case TerrainType::Hills: cost = 2.0f; break;
            case TerrainType::Mountains: cost = 3.0f; break;
            case TerrainType::Forest: cost = 2.0f; break;
            case TerrainType::Jungle: cost = 2.0f; break;
            case TerrainType::Marsh: cost = 2.0f; break;
            default: break;
        }
