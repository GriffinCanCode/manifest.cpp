...
public:
    FractalNoise(std::unique_ptr<NoiseGenerator> base_noise, 
                 int octaves = 4, 
...
private:
    ...
    void generate_terrain_types(MapType& map) {
        map.for_each_tile([this](Tile& tile) {
            ...
            if (elevation < params_.sea_level) {
                terrain = elevation < params_.sea_level * 0.5f ? TerrainType::Ocean : TerrainType::Coast;
            ...
            switch (terrain) {
                case TerrainType::Grassland: fertility = 200; break;
                case TerrainType::Plains: fertility = 150; break;
                case TerrainType::Forest: fertility = 120; break;
                case TerrainType::Jungle: fertility = 100; break;
                case TerrainType::Hills: fertility = 80; break;
                case TerrainType::Desert: fertility = 20; break;
                case TerrainType::Tundra: fertility = 30; break;
                case TerrainType::Coast: fertility = 60; break;
                default: fertility = 10; break;
            }
    ...
    void generate_resources(MapType& map) {
        map.for_each_tile([this](Tile& tile) {
            ...
            const HexCoordinate& coord = tile.coordinate();
            float resource_value = resource_noise_->sample(
                static_cast<float>(coord.q) * 0.1f, 
            ...
            switch (tile.terrain()) {
                ...
                case TerrainType::Mountains:
                    if (resource_value > 0.7f) resource = ResourceType::Gold;
                    else if (resource_value > 0.3f) resource = ResourceType::Iron;
                    else resource = ResourceType::Stone;
                    break;
                    
                case TerrainType::Plains:
                case TerrainType::Grassland:
                    if (resource_value > 0.5f) resource = ResourceType::Wheat;
                    else resource = ResourceType::Cattle;
                    break;
