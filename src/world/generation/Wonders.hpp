...
class NaturalWonderSystem {
   ...
   public:
    ...
    void generate_wonders(TileMap& map, std::size_t target_count = 12) {
        ...
        for (std::size_t type_idx = 0;
             type_idx < wonder_types_.size() && tile_to_wonder_.size() < target_count; ++type_idx) {
    ...
    const std::vector<WonderDescriptor>& wonder_types() const noexcept { return wonder_types_; }

    std::optional<std::size_t> get_wonder_at_tile(TileId tile_id) const {
        auto it = tile_to_wonder_.find(tile_id);
        return it != tile_to_wonder_.end() ? std::optional<std::size_t>{it->second} : std::nullopt;
    }

   private:
    void initialize_wonder_types() {
        wonder_types_ = {// Mountain Wonders
    ...
    std::vector<TileId> find_wonder_locations(const TileMap& map, const WonderDescriptor& wonder) {
    ...
    bool is_too_close_to_existing_wonder(const TileMap& map, TileId tile_id) {
        ...
        for (const auto& [wonder_tile_id, wonder_type_idx] : tile_to_wonder_) {
            ...
            std::int32_t distance =
                TileMap::distance(tile->coordinate(), wonder_tile->coordinate());
        ...
        for (TileId nearby_id : nearby_tiles) {
            ...
            switch (wonder.type) {
                ...
                case WonderType::Lake:
                    ...
                    if (nearby->is_land()) {
                        nearby->set_fertility(
                            std::min(255, static_cast<int>(current_fertility + 30)));
                    }
                    break;
