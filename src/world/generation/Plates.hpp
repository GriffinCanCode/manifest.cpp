...
enum class PlateType : std::uint8_t {
    Continental,
    Oceanic
};
...
private:
    std::vector<Vec2f> generate_plate_centers(const TileMap& map, std::size_t count) {
        ...
        for (std::size_t attempts = 0; attempts < count * 100 && centers.size() < count; ++attempts) {
    ...
    void assign_tiles_to_plates(TileMap& map) {
        map.for_each_tile([this](const Tile& tile) {
            ...
            if (closest_plate.is_valid()) {
                ...
                // Add tile to plate
                auto plate_it = std::find_if(plates_.begin(), plates_.end(),
