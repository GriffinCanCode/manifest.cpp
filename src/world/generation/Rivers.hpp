...
private:
    ...
    float calculate_flow_rate(const Tile& current, const Tile& next) {
        ...
        switch (current.terrain()) {
            case TerrainType::Mountains: terrain_multiplier = 2.0f; break;
            case TerrainType::Hills: terrain_multiplier = 1.5f; break;
            case TerrainType::Forest: terrain_multiplier = 1.2f; break;
            case TerrainType::Jungle: terrain_multiplier = 1.3f; break;
            case TerrainType::Desert: terrain_multiplier = 0.3f; break;
            default: break;
        }
            ...
            for (const auto& segment : river.segments()) {
                ...
                for (const TileId& neighbor_id : river_tile->neighbors()) {
                    ...
                    if (neighbor->terrain() == TerrainType::Plains || 
                        ...
                        neighbor->set_fertility(std::min(255, static_cast<int>(current_fertility + 40)));
                    }
                }
            }
        }
