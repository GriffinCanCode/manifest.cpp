...
public:
    ...
    void rotate_pitch(float angle) {
        ...
        Mat3f rotation{
            right.x() * right.x() * (1 - c) + c,           right.x() * right.y() * (1 - c) - right.z() * s, right.x() * right.z() * (1 - c) + right.y() * s,
            right.y() * right.x() * (1 - c) + right.z() * s, right.y() * right.y() * (1 - c) + c,           right.y() * right.z() * (1 - c) - right.x() * s,
            right.z() * right.x() * (1 - c) - right.y() * s, right.z() * right.y() * (1 - c) + right.x() * s, right.z() * right.z() * (1 - c) + c
        };
    ...
    // Utility functions
    Vec3f screen_to_world(const Vec2f& screen_coords, const Vec2f& viewport_size, float depth = 1.0f) const {
        // Convert screen coordinates to NDC
        Vec3f ndc{
            (screen_coords.x() / viewport_size.x()) * 2.0f - 1.0f,
            1.0f - (screen_coords.y() / viewport_size.y()) * 2.0f,
            depth * 2.0f - 1.0f
        };
