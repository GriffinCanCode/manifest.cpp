...
RenderStats HexRenderer::get_stats() const {
    ...
    if (use_procedural_ && procedural_renderer_) {
        stats.vertices_rendered = static_cast<std::uint32_t>(procedural_renderer_->instance_count() * 7); // 7 vertices per hex
        stats.triangles_rendered = static_cast<std::uint32_t>(procedural_renderer_->instance_count() * 6); // 6 triangles per hex
        stats.draw_calls = procedural_renderer_->instance_count() > 0 ? 1 : 0; // Single instanced draw call
    }
