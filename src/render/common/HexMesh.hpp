...
private:
    ...
    static void generate_hex_face(HexMesh& mesh, const Vec2f& center, float height, bool is_top) {
        ...
        mesh.vertices.push_back(Vertex{
            .position = Vec3f{center.x(), height, center.y()},
        ...
        for (std::size_t i = 0; i < HEX_SIDES; ++i) {
            ...
            std::uint32_t next_idx = static_cast<std::uint32_t>(start_vertex + 1 + ((i + 1) % HEX_SIDES));
    ...
    static void generate_hex_sides(HexMesh& mesh, const Vec2f& center, float height) {
        ...
        for (std::size_t i = 0; i < HEX_SIDES; ++i) {
            ...
            mesh.indices.insert(mesh.indices.end(), {
                top_curr, bottom_curr, top_next,
                top_next, bottom_curr, bottom_next
            });
        }
    }
    ...
    static Vec4f get_terrain_color(TerrainType terrain, float elevation) {
        switch (terrain) {
            ...
            case TerrainType::Hills:
                return Vec4f{0.5f + (0.7f - 0.5f) * elevation, 0.6f + (0.7f - 0.6f) * elevation, 0.3f + (0.6f - 0.3f) * elevation, 1.0f};
            case TerrainType::Mountains:
                return Vec4f{0.6f + (0.9f - 0.6f) * elevation, 0.6f + (0.9f - 0.6f) * elevation, 0.5f + (0.9f - 0.5f) * elevation, 1.0f};
