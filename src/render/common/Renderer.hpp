...
class Renderer {
   public:
    ...
    virtual void bind_vertex_buffer(BufferHandle buffer, std::uint32_t binding = 0,
...
class ScopedRenderPass {
   ...
   public:
    ...
    ~ScopedRenderPass() { renderer_->end_render_pass(); }
...
class ScopedDebugGroup {
   ...
   public:
    ScopedDebugGroup(Renderer* renderer, std::string_view name) : renderer_{renderer} {
        renderer_->push_debug_group(name);
    }

    ~ScopedDebugGroup() { renderer_->pop_debug_group(); }
...
#define SCOPED_RENDER_PASS(renderer, target, ...) \
    auto _render_pass = ScopedRenderPass {        \
        renderer, target, ##__VA_ARGS__           \
    }

#define SCOPED_DEBUG_GROUP(renderer, name) \
    auto _debug_group = ScopedDebugGroup { \
        renderer, name                     \
    }
...
constexpr std::string_view to_string(RendererError error) noexcept {
    switch (error) {
        case RendererError::InitializationFailed:
            return "Initialization failed";
        case RendererError::DeviceLost:
            return "Device lost";
        case RendererError::OutOfMemory:
            return "Out of memory";
        case RendererError::InvalidHandle:
            return "Invalid handle";
        case RendererError::InvalidState:
            return "Invalid state";
        case RendererError::CompilationFailed:
            return "Compilation failed";
        case RendererError::UnsupportedFormat:
