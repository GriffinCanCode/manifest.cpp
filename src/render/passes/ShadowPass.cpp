...
Result<void> ShadowPass::create_shadow_pipeline() {
    ...
    static const VertexBinding shadow_binding{
        .binding = 0,
...
void ShadowPass::calculate_cascade_matrices(const PassContext& context) {
    ...
    for (std::uint32_t i = 0; i < CASCADE_COUNT; ++i) {
        ...
        auto frustum_corners = calculate_frustum_corners(context.view_projection_matrix, split_near, split_far);
        
        // Create orthographic projection that tightly fits the frustum
        Mat4f light_view_proj = create_orthographic_projection(shadow_uniforms_.light_direction, frustum_corners);
    ...
    shadow_uniforms_.cascade_splits = Vec4f{cascade_splits_[0], cascade_splits_[1], 
                                           cascade_splits_[2], cascade_splits_[3]};
}
...
Mat4f ShadowPass::create_orthographic_projection(const Vec3f& light_dir, 
    ...
    Mat4f light_proj = Mat4f::orthographic(min_bounds.x(), max_bounds.x(),
...
std::array<Vec3f, 8> ShadowPass::calculate_frustum_corners(const Mat4f& view_proj, 
                                                          float near_dist, float far_dist) {
    // This is a simplified implementation
    // A full implementation would properly calculate frustum corners from the view projection matrix
    ...
    for (std::size_t i = 0; i < 8; ++i) {
        Vec4f world_corner = inv_view_proj * Vec4f{ndc_corners[i].x(), ndc_corners[i].y(), 
                                                  ndc_corners[i].z(), 1.0f};
