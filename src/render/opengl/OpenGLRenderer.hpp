...
public:
    ...
    Result<BufferHandle> create_buffer(const BufferDesc& desc, std::span<const std::byte> initial_data) override {
        GLuint buffer_id;
    ...
    Result<PipelineHandle> create_pipeline(const PipelineDesc& desc) override {
        ...
        pipelines_[handle] = PipelineInfo{
            .program = program,
            .desc = desc,
            .debug_name = std::string{desc.debug_name},
    ...
    void draw(const DrawCommand& command) override {
        GLenum primitive = primitive_topology_to_gl(
            pipelines_[current_pipeline_].desc.render_state.topology);
    ...
    void draw_indexed(const DrawIndexedCommand& command) override {
        ...
        glDrawElementsInstancedBaseVertex(primitive, command.index_count, GL_UNSIGNED_INT,
    ...
    API get_api() const noexcept override {
        return API::OpenGL;
    }
...
private:
    ...
    void reload_shader(ShaderHandle handle) {
        ...
        for (auto& [pipeline_handle, pipeline_info] : pipelines_) {
            bool uses_shader = std::find(pipeline_info.shaders.begin(), 
                                       pipeline_info.shaders.end(), handle) != pipeline_info.shaders.end();
    ...
    static GLenum buffer_usage_to_gl_target(BufferUsage usage) {
        switch (usage) {
            ...
            case BufferUsage::Staging: return GL_ARRAY_BUFFER;
    ...
    Result<TextureHandle> create_texture(const TextureDesc& desc, std::span<const std::byte> initial_data) override {
        return std::unexpected(RendererError::UnsupportedFormat);
    }
    
    Result<RenderTargetHandle> create_render_target(std::span<const TextureHandle> color_attachments, TextureHandle depth_attachment) override {
        return std::unexpected(RendererError::UnsupportedFormat);
    }
    ...
    Result<void> update_buffer(BufferHandle handle, std::size_t offset, std::span<const std::byte> data) override {
        return std::unexpected(RendererError::InvalidHandle);
    }
    
    Result<void> update_texture(TextureHandle handle, std::uint32_t mip_level, std::span<const std::byte> data) override {
        return std::unexpected(RendererError::InvalidHandle);
    }
